...
#
# Install Debian GNU/Linux 9 buster to a native ZFS root filesystem
#
...
cp -va /etc/network/interfaces /target/etc/network/interfaces
cp -va /etc/resolv.conf /target/etc/resolv.conf
...
chroot /target /usr/bin/apt install -y linux-headers-$(uname -r)
chroot /target /usr/bin/apt install -y firmware-linux-nonfree
chroot /target /usr/bin/apt install -y dpkg-dev
chroot /target /usr/bin/apt install -y  spl spl-dkms

chroot /target /usr/bin/apt-get install --yes linux-image-amd64 grub2-common $GRUBPKG zfs-initramfs zfs-dkms
grep -q zfs /target/etc/default/grub || perl -i -pe 's/quiet/boot=zfs quiet rootdelay=5/' /target/etc/default/grub
